...
class Evaluator:
    ...
    def mean_intersection_over_union(self, percent=False) -> tuple[Tensor, Tensor]:
        iou = self.intersection_over_union(percent)
        miou = torch.nanmean(iou)
        return iou, miou

    def frequency_weighted_intersection_over_union(self) -> Tensor:
        freq = torch.sum(self.confusion_matrix, dim=0) / torch.sum(self.confusion_matrix)
        iou = self.intersection_over_union()
        fwiou = (freq[freq > 0] * iou[freq > 0]).sum()
        return fwiou

    def _generate_matrix(self, gt_batch: Tensor, pred_batch: Tensor) -> Tensor:
    ...
    def update_matrix(self, gt_batch: Tensor, pred_batch: Tensor):
